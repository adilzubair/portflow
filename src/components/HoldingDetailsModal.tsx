...
export default function HoldingDetailsModal({ holding, inrToAedRate, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto overscroll-contain p-4 sm:items-center" onClick={onClose}>
      ...
      <div
        className="relative my-6 max-h-[calc(100dvh-3rem)] w-full max-w-5xl overflow-x-hidden overflow-y-auto rounded-2xl border border-border-default bg-bg-card p-4 shadow-xl sm:max-h-[90vh] sm:p-6"
        onClick={(event) => event.stopPropagation()}
      >
        <div className="flex items-start justify-between gap-4">
          <div className="min-w-0">
            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-text-muted">Holding details</p>
            <h2 className="mt-2 truncate font-display text-2xl font-semibold tracking-[-0.03em] text-text-primary">{holding.assetName}</h2>
            <div className="mt-2 flex flex-wrap items-center gap-2 text-xs text-text-muted">
              {holding.ticker ? <span className="rounded-md bg-bg-elevated px-2 py-1 font-mono text-text-secondary">{holding.ticker}</span> : null}
          ...
          <button
            type="button"
            onClick={() => { tap(); onClose(); }}
            className="shrink-0 rounded-xl border border-border-default bg-bg-card px-3 py-2 text-sm font-medium text-text-secondary transition hover:bg-bg-elevated hover:text-text-primary"
          >
            Close
          </button>
        </div>
        ...
        <div className="mt-6 space-y-5 sm:hidden">
          ...
          <section className="rounded-2xl border border-border-default bg-bg-elevated p-4">
          ...
          <section className="rounded-2xl border border-border-default bg-bg-card p-4">
            <h3 className="text-sm font-semibold text-text-primary">Asset info</h3>
          ...
          <section className="rounded-2xl border border-border-default bg-bg-card p-4">
            <div className="flex items-center justify-between gap-3">
              ...
              <span className="text-xs text-text-muted">{purchases.length} entries</span>
            </div>
            ...
            {purchases.length ? (
              <div className="mt-4 space-y-3">
                {purchases.map((purchase, index) => {
                  ...
                  return (
                    <div key={`${purchase.date}-${index}`} className="rounded-2xl border border-border-default bg-bg-elevated p-3">
                      <div className="text-xs font-medium uppercase tracking-wider text-text-muted">{purchase.date}</div>
              ...
              <p className="mt-4 text-sm text-text-muted">No purchase history recorded for this holding.</p>
            )}
          </section>
        </div>
        ...
        <div className="mt-6 hidden gap-6 sm:grid lg:grid-cols-[280px,minmax(0,1fr)]">
          <div className="space-y-4">
            <div className="grid gap-2 rounded-2xl border border-border-default bg-bg-elevated p-3 sm:p-4">
                  ...
                  <table className="min-w-full border-collapse text-sm">
                    ...
                    <tbody>
                      {purchases.map((purchase, index) => {
                        ...
                        const price = toNumber(purchase.price);
                        return (
                          <tr key={`${purchase.date}-${index}`} className="border-b border-border-subtle last:border-b-0">
                            <td className="px-3 py-3 text-text-secondary">{purchase.date}</td>
                            <td className="px-3 py-3 font-mono text-text-secondary">
                              {quantity < 1 ? quantity.toFixed(7) : quantity.toLocaleString()}
                            </td>
                            <td className="px-3 py-3 font-mono text-text-secondary">{formatMoney(price, holding.currency)}</td>
                            {holding.currency === "INR" ? (
...
function StatCard({ title, value }: { title: string; value: string }) {
  return (
    <div className="rounded-2xl border border-border-default bg-bg-elevated px-4 py-3">
